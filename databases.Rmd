...
At times I'll ask you to post code/syntax on this shared document: [https://etherpad.net/p/scf-sql-2021](https://etherpad.net/p/scf-sql-2021).
...
The database we'll work with as our example is meta-data on questions and answers on the Stack Overflow code discussion website from 2016. We don't have the full text of the questions or the answers, but we do have the question and answer titles, some information about how popular the questions and answers were, information about the users posting questions or answers, and the topics (tags) associated with each question. Each question may have zero or more answers and each question may have one or more tags indicating what the question is above.
...
These approaches have a lot of disadvantages. Let's list them as a group:
...
In our Stack Overflow example, the primary keys would presumably be: questions.ID, answers.ID, users.ID, and for QuestionsTags table two fields: {QuestionsTags.questionID, QuestionsTags.tag}.

Some examples of foreign keys would be:
 - ownerID as the foreign key in Questions for joining with Users on the users primary key (ID)
 - questionID as the foreign key in Answers for joining with Questions on the questions primary key (ID)
...
I've obtained data from [Stack Overflow](https://stackoverflow.com), the popular website for asking coding questions, and placed it into a normalized database (which we'll define later). The SQLite version (also in CSVs as one CSV per table) has metadata (i.e., it lacks the actual text of the questions and answers) on all of the questions and answers posted in 2016.

Now let's consider the organization of the Stack Overflow data. Each question may have multiple answers and each question may have multiple (topic) tags.
...
First suppose we want to find the number of users who have asked questions:
...
How about finding the users who have asked at least 50 questions? How do we determine the number of questions per user? What are we grouping by?
...
dbGetQuery(db, "select ownerid, count(*) as n from questions group by ownerid having n >= 50 order by n")
```

What if we wanted actual information about the users, not just their ID? We need information from the users table.

```{r, eval=FALSE, echo=FALSE}
dbGetQuery(db, "select ownerid, count(*) as n from questions join users on
               questions.ownerid = users.userid
               group by ownerid having n >= 50 order by n")
```

What if we wanted to count the number of users who have asked at least 50 questions? We'll discuss one way to do this later.
...
***Challenge***:  Suppose the Stack Overflow database has 10,000 questions and 30,000 answers.
...
***Challenge***: Select the questions that have "java" in their titles using regular expression syntax (TODO: NEED TO TRY THIS)
